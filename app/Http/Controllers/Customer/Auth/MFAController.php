...
class MFAController extends Controller
{
    ...
    public function checkVerification(Request $request)
    {
        ...
        if (! $customer) {
            if (\App\Models\Owner::where('email', $request->input('email'))->exists()) {
                return response()->json([
                    'status' => 'ERROR',
                    'errors' => 'This account is registered as a Fleet Owner. Please sign in through the Fleet Owner Portal.',
                    'redirect_portal' => '/owner/login?email='.urlencode($request->input('email')),
                    'portal_name' => 'Fleet Owner Portal',
                ], 401);
            }
        ...
        if (Hash::check($request->input('password'), $customer->password)) {
            ...
            if ($isEmailAuth && ! $isMfa) {
                ...
                try {
                    EmailVerificationJob::dispatchSync($customer, $code);
                    $codeSent = true;
                } catch (\Throwable $e) {
                    Log::warning('Customer email verification send failed: '.$e->getMessage());
                    $codeSent = true;
                }
            }
    ...
    public function resendCode(Request $request)
    {
        ...
        if ($customer && Hash::check($request->input('password'), $customer->password) && $customer->is_email_authentication_enabled) {
            ...
            try {
                ...
                Log::warning('Resend customer email verification failed: '.$e->getMessage());
            }
