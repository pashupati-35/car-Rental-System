...
const activateMfa = async () => {
  ...
  try {
    ...
    if (res.data.status === 'OK') {
      ...
      mfaSuccess.value = 'Two-Factor Authentication is now active on your Customer account!'
    } else {
...
<template>
  <AppLayout>
    <Head title="Customer Profile & Security" />
    ...
    <div class="max-w-4xl mx-auto space-y-6">
      ...
      <div class="p-6 rounded-2xl bg-white dark:bg-gray-900 border border-gray-100 dark:border-gray-800 shadow-sm">
        ...
        <p class="text-xs text-gray-500 mt-0.5">
          Manage your contact details and billing address for car rentals
        </p>
        ...
        <form class="mt-5 space-y-4" @submit.prevent="updateProfile">
          ...
          <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div>
              <label class="block text-xs font-semibold text-gray-700 dark:text-gray-300 mb-1">Residential Address</label>
              <input
                v-model="profileForm.address"
                type="text"
                placeholder="456 Sunset Blvd"
                class="w-full px-3.5 py-2.5 rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 text-sm focus:outline-none focus:ring-2 focus:ring-purple-500"
              >
            </div>
          </div>
          ...
          <div class="flex justify-end">
            <button
              ...
              class="px-5 py-2.5 rounded-xl bg-purple-600 hover:bg-purple-700 text-white font-semibold text-xs shadow-md shadow-purple-500/20 disabled:opacity-50"
            >
      ...
      <div class="p-6 rounded-2xl bg-white dark:bg-gray-900 border border-gray-100 dark:border-gray-800 shadow-sm">
        ...
        <p class="text-xs text-gray-500 mt-0.5">
          Ensure your customer account remains secure
        </p>
        ...
        <form class="mt-5 space-y-4" @submit.prevent="updatePassword">
          <div>
            ...
            <input
              ...
              type="password"
              required
              class="w-full max-w-md px-3.5 py-2.5 rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 text-sm focus:outline-none focus:ring-2 focus:ring-purple-500"
            >
          </div>
      ...
      <div class="p-6 rounded-2xl bg-white dark:bg-gray-900 border border-gray-100 dark:border-gray-800 shadow-sm">
        <div class="flex items-start justify-between">
          <div>
            <h3 class="font-bold text-lg text-gray-900 dark:text-white flex items-center gap-2">
              <i class="ri-shield-keyhole-line text-purple-600" />
              Two-Factor Authentication (MFA)
            </h3>
            <p class="text-xs text-gray-500 mt-1 max-w-xl">
              Add 2FA security to your customer account for safer reservations and payment transactions.
            </p>
          </div>
        ...
        <div
          v-if="showMfaSetup"
          class="mt-6 p-6 rounded-2xl bg-purple-50/50 dark:bg-purple-950/20 border border-purple-100 dark:border-purple-900/50 space-y-4"
        >
          ...
          <div class="flex flex-col sm:flex-row items-center gap-6">
            ...
            <div class="space-y-3 text-xs flex-1">
              ...
              <div class="p-2.5 rounded-lg bg-white dark:bg-gray-800 font-mono font-bold text-purple-600 text-sm border border-gray-200 dark:border-gray-700 select-all">
                {{ secretKey }}
              </div>
              <div class="space-y-1.5 pt-2">
                ...
                <div class="flex gap-2">
                  <input
                    ...
                    maxlength="6"
                    placeholder="123456"
                    class="px-3 py-2 rounded-xl border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-800 font-mono text-center tracking-widest text-sm focus:outline-none focus:ring-2 focus:ring-purple-500"
                  >
                  <button
                    :disabled="mfaLoading"
                    class="px-4 py-2 rounded-xl bg-purple-600 hover:bg-purple-700 text-white font-semibold text-xs transition-colors disabled:opacity-50"
                    @click="activateMfa"
                  >
        ...
        <div class="mt-6 pt-4 border-t border-gray-100 dark:border-gray-800 flex gap-3">
          <button
            ...
            :disabled="mfaLoading"
            class="px-4 py-2.5 rounded-xl bg-purple-600 hover:bg-purple-700 text-white font-semibold text-xs shadow-md shadow-purple-500/20 transition-all disabled:opacity-50"
            @click="initMfaSetup"
          >
