...
<template>
  <div class="bg-white dark:bg-slate-900 rounded-3xl border border-slate-200/80 dark:border-slate-800 shadow-xs overflow-hidden">
    <!-- Empty State -->
    <div v-if="cars.length === 0" class="py-16 text-center px-4">
      <i class="ri-car-line text-4xl text-slate-300 dark:text-slate-700 mb-2 inline-block" />
      <p class="text-sm font-bold text-slate-700 dark:text-slate-300">No cars found</p>
      <p class="text-xs text-slate-500 mt-1 max-w-sm mx-auto">No vehicle records match your selected status or filter query.</p>
    </div>

    <template v-else>
      <!-- Mobile Cards (xs, sm) -->
      <div class="block md:hidden divide-y divide-slate-100 dark:divide-slate-800">
        <div v-for="car in cars" :key="car.id" class="p-4 space-y-3">
          <div class="flex items-start justify-between gap-2">
            <div class="flex items-center gap-3">
              <div class="w-12 h-12 rounded-2xl bg-slate-100 overflow-hidden shrink-0 border border-slate-200">
                <img v-if="car.image" :src="car.image" class="w-full h-full object-cover" />
              ...
              <div>
                ...
                <p class="text-[11px] font-mono text-slate-400">{{ car.car_number || car.plate_number || 'No Plate' }}</p>
              </div>
            </div>
          ...
          <div class="grid grid-cols-2 gap-2 text-xs py-2 px-3 bg-slate-50 dark:bg-slate-800/40 rounded-xl">
            <div>
              ...
              <span v-else class="text-slate-400">Direct Fleet</span>
            </div>
            <div>
      ...
      <div class="hidden md:block overflow-x-auto">
        <table class="w-full text-left text-xs">
          <thead class="bg-slate-50 dark:bg-slate-800/60 text-slate-500 uppercase font-bold border-b border-slate-200/80 dark:border-slate-800">
            <tr>
              <th class="py-3.5 px-5">Vehicle Name & Model</th>
              <th class="py-3.5 px-5">Fleet Owner</th>
              <th class="py-3.5 px-5">Assigned Driver</th>
              <th class="py-3.5 px-5">Rate / Day</th>
              <th class="py-3.5 px-5">Approval Status</th>
              <th class="py-3.5 px-5 text-right">Admin Actions</th>
            </tr>
          </thead>
          <tbody class="divide-y divide-slate-100 dark:divide-slate-800 text-slate-700 dark:text-slate-300">
            <tr v-for="car in cars" :key="car.id" class="hover:bg-slate-50/60 dark:hover:bg-slate-800/30 transition-colors">
              <!-- Vehicle -->
              <td class="py-4 px-5">
                <div class="flex items-center gap-3">
                  <div class="w-12 h-12 rounded-2xl bg-slate-100 overflow-hidden shrink-0 border border-slate-200">
              ...
              <td class="py-4 px-5">
                ...
                <span v-else class="text-slate-400 font-medium">Platform Direct Fleet</span>
              </td>

              <!-- Driver -->
              <td class="py-4 px-5">
                <span v-if="car.driver" class="font-medium text-slate-800 dark:text-slate-200 block">
                  {{ car.driver.name }}
                </span>
                <span v-else class="text-slate-400 italic">Self-Drive / None</span>
              </td>
