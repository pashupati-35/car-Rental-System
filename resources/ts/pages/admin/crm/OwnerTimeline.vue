  ...
  <CrmLayout>
    ...
    <div class="space-y-6 pb-12">
      ...
      <div class="flex items-center gap-2 text-xs font-medium text-slate-500">
        <Link href="/crm/owners" class="hover:text-emerald-600 transition flex items-center gap-1">
        ...
        <div class="flex items-center gap-4">
          ...
          <div>
            <div class="flex items-center gap-3">
              ...
              <span v-if="preference?.vip_partner" class="px-2 py-0.5 rounded-md text-[10px] font-bold bg-purple-100 text-purple-800 dark:bg-purple-950 dark:text-purple-300">
                VIP Partner
              </span>
            </div>
            <div class="flex flex-wrap items-center gap-x-4 gap-y-1 text-xs text-slate-500 mt-1">
              ...
              <span v-if="owner.contact_number || owner.mobile" class="flex items-center gap-1">
                <i class="ri-phone-line" /> {{ owner.contact_number || owner.mobile }}
              </span>
              <span v-if="owner.address" class="flex items-center gap-1">
      ...
      <div v-if="activeTab === 'fleet'" class="space-y-4">
        <div v-if="cars.length > 0" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          <div
            v-for="car in cars"
            :key="car.id"
          ...
          >
            <div>
              ...
              <p class="text-xs text-slate-400 mt-0.5">Brand: {{ car.brand?.name || 'Standard' }}</p>
            </div>
        ...
        <div v-else class="text-center py-12 bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 p-6 text-slate-500">
      ...
      <div v-if="activeTab === 'interactions'" class="space-y-4">
        <div v-if="interactions.length > 0" class="space-y-3">
          ...
          >
            <div class="w-10 h-10 rounded-xl flex items-center justify-center text-lg shrink-0" :class="interactionIcons[interaction.type] || 'ri-message-line'">
              <i :class="interactionIcons[interaction.type]?.split(' ')[0] || 'ri-message-line'" />
            </div>
            <div class="flex-1">
              <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-1">
                <h4 class="font-bold text-slate-900 dark:text-white text-sm">{{ interaction.subject }}</h4>
                <span class="text-[11px] text-slate-400">{{ new Date(interaction.interaction_date).toLocaleString() }}</span>
              </div>
              <p class="text-xs text-slate-600 dark:text-slate-300 mt-1 whitespace-pre-wrap">{{ interaction.details }}</p>
              <div v-if="interaction.admin" class="text-[11px] text-indigo-600 dark:text-indigo-400 mt-2 font-medium">
      ...
      <div v-if="activeTab === 'bookings'" class="space-y-4">
        <div v-if="recent_bookings.length > 0" class="overflow-x-auto bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-xs">
          <table class="w-full text-left text-xs">
            ...
            <tbody class="divide-y divide-slate-100 dark:divide-slate-800">
              <tr v-for="b in recent_bookings" :key="b.id" class="hover:bg-slate-50/50 dark:hover:bg-slate-800/50">
                <td class="p-3.5 font-mono font-bold">{{ b.booking_number || '#' + b.id }}</td>
                <td class="p-3.5 font-semibold text-slate-900 dark:text-white">{{ b.car?.car_name || b.car?.name }}</td>
                <td class="p-3.5">{{ b.customer?.name || 'Client #' + b.customer_id }}</td>
                <td class="p-3.5 text-slate-500">{{ b.start_date }} &rarr; {{ b.end_date }}</td>
                <td class="p-3.5 font-bold text-emerald-600">${{ b.total_price || b.amount || 0 }}</td>
                <td class="p-3.5">
                  <span class="px-2 py-0.5 rounded text-[10px] font-bold uppercase" :class="b.status === 'confirmed' ? 'bg-emerald-100 text-emerald-800' : 'bg-slate-100 text-slate-700'">
                    {{ b.status || 'Pending' }}
                  </span>
                </td>
              </tr>
      ...
      <div v-if="activeTab === 'drivers'" class="space-y-4">
        <div v-if="drivers.length > 0" class="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          >
            <div>
              <h4 class="font-bold text-slate-900 dark:text-white text-sm">{{ d.name || `${d.first_name || ''} ${d.last_name || ''}`.trim() }}</h4>
              <p class="text-xs text-slate-400 mt-0.5">{{ d.email }} &bull; {{ d.phone || d.mobile }}</p>
            </div>
      ...
      <div v-if="activeTab === 'tickets'" class="space-y-4">
        <div v-if="support_tickets.length > 0" class="space-y-3">
          <div
          ...
          >
            <div>
              <div class="flex items-center gap-2">
                ...
                <span class="px-2 py-0.5 rounded text-[10px] font-bold uppercase" :class="t.status === 'resolved' ? 'bg-emerald-100 text-emerald-800' : 'bg-rose-100 text-rose-800'">
                  {{ t.status }}
                </span>
              </div>
              <h4 class="font-bold text-slate-900 dark:text-white text-sm mt-1">{{ t.subject }}</h4>
            </div>
            <Link :href="`/crm/tickets/${t.id}`" class="px-3 py-1.5 rounded-lg text-xs font-semibold text-indigo-600 dark:text-indigo-400 bg-indigo-50 dark:bg-indigo-950 hover:bg-indigo-100">
              View Ticket &rarr;
            </Link>
          </div>
        </div>
      ...
      <div v-if="activeTab === 'tasks'" class="space-y-4">
        <div v-if="tasks.length > 0" class="space-y-3">
          ...
          >
            <div>
              <h4 class="font-bold text-slate-900 dark:text-white text-sm" :class="{ 'line-through text-slate-400': task.status === 'completed' }">
                {{ task.title }}
              </h4>
    ...
    <div v-if="showInteractionModal" class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-xs">
      <div class="bg-white dark:bg-slate-900 rounded-2xl max-w-lg w-full p-6 border border-slate-200 dark:border-slate-800 shadow-xl space-y-4">
        <div class="flex items-center justify-between">
          <h3 class="font-bold text-slate-900 dark:text-white text-base">Log Partner Interaction</h3>
          <button type="button" @click="showInteractionModal = false" class="text-slate-400 hover:text-slate-600 p-1 cursor-pointer">
            <i class="ri-close-line text-lg" />
          </button>
        </div>

        <form class="space-y-4" @submit.prevent="submitInteraction">
          <div class="grid grid-cols-2 gap-3">
            <div>
              <label class="block text-xs font-semibold text-slate-700 dark:text-slate-300 mb-1">Type</label>
              <select v-model="interactionForm.type" class="w-full px-3 py-2 rounded-xl border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800 text-xs">
                <option value="call">Phone Call</option>
                <option value="meeting">Fleet Meeting</option>
            ...
            <div>
              ...
              <input v-model="interactionForm.interaction_date" type="datetime-local" class="w-full px-3 py-2 rounded-xl border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800 text-xs">
            </div>
          </div>

          <div>
            <label class="block text-xs font-semibold text-slate-700 dark:text-slate-300 mb-1">Subject</label>
            <input v-model="interactionForm.subject" type="text" required placeholder="e.g. Monthly Payout Audit, New Vehicle Inspection" class="w-full px-3 py-2 rounded-xl border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800 text-xs">
          </div>

          <div>
            <label class="block text-xs font-semibold text-slate-700 dark:text-slate-300 mb-1">Notes / Details</label>
            <textarea v-model="interactionForm.details" rows="4" required placeholder="Provide details of the conversation..." class="w-full px-3 py-2 rounded-xl border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800 text-xs"></textarea>
          </div>

          <div class="flex justify-end gap-2 pt-2">
            <button type="button" @click="showInteractionModal = false" class="px-4 py-2 rounded-xl border border-slate-200 dark:border-slate-700 text-xs">Cancel</button>
            <button type="submit" :disabled="interactionForm.processing" class="px-4 py-2 rounded-xl bg-indigo-600 hover:bg-indigo-700 text-white font-semibold text-xs">Save Log</button>
          </div>
        </form>
      </div>
    </div>
    ...
    <!-- Modal: Add Task -->
    <div v-if="showTaskModal" class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-xs">
      <div class="bg-white dark:bg-slate-900 rounded-2xl max-w-lg w-full p-6 border border-slate-200 dark:border-slate-800 shadow-xl space-y-4">
        <div class="flex items-center justify-between">
          <h3 class="font-bold text-slate-900 dark:text-white text-base">Schedule Partner Task</h3>
          <button type="button" @click="showTaskModal = false" class="text-slate-400 hover:text-slate-600 p-1 cursor-pointer">
            <i class="ri-close-line text-lg" />
          </button>
        </div>
        ...
        <form class="space-y-4" @submit.prevent="submitTask">
          <div>
            ...
            <input v-model="taskForm.title" type="text" required placeholder="e.g. Verify vehicle insurance renewal" class="w-full px-3 py-2 rounded-xl border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800 text-xs">
          </div>

          <div class="grid grid-cols-2 gap-3">
            <div>
              <label class="block text-xs font-semibold text-slate-700 dark:text-slate-300 mb-1">Due Date</label>
              <input v-model="taskForm.due_date" type="datetime-local" class="w-full px-3 py-2 rounded-xl border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800 text-xs">
            </div>
            <div>
              <label class="block text-xs font-semibold text-slate-700 dark:text-slate-300 mb-1">Priority</label>
              <select v-model="taskForm.priority" class="w-full px-3 py-2 rounded-xl border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800 text-xs">
          ...
          <div>
            ...
            <textarea v-model="taskForm.description" rows="3" placeholder="Additional task instructions..." class="w-full px-3 py-2 rounded-xl border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800 text-xs"></textarea>
          </div>

          <div class="flex justify-end gap-2 pt-2">
            <button type="button" @click="showTaskModal = false" class="px-4 py-2 rounded-xl border border-slate-200 dark:border-slate-700 text-xs">Cancel</button>
            <button type="submit" :disabled="taskForm.processing" class="px-4 py-2 rounded-xl bg-indigo-600 hover:bg-indigo-700 text-white font-semibold text-xs">Schedule Task</button>
          </div>
        </form>
      </div>
    </div>
    ...
    <div v-if="showPreferenceModal" class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-xs">
      <div class="bg-white dark:bg-slate-900 rounded-2xl max-w-lg w-full p-6 border border-slate-200 dark:border-slate-800 shadow-xl space-y-4">
        <div class="flex items-center justify-between">
          <h3 class="font-bold text-slate-900 dark:text-white text-base">Partner Terms & Payout Settings</h3>
          <button type="button" @click="showPreferenceModal = false" class="text-slate-400 hover:text-slate-600 p-1 cursor-pointer">
            <i class="ri-close-line text-lg" />
          </button>
        </div>
        ...
        <form class="space-y-4" @submit.prevent="submitPreferences">
          <div class="grid grid-cols-2 gap-3">
            <div>
              ...
              <select v-model="prefForm.partner_tier" class="w-full px-3 py-2 rounded-xl border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800 text-xs">
            ...
            <div>
              ...
              <input v-model="prefForm.commission_rate" type="number" step="0.5" min="0" max="100" class="w-full px-3 py-2 rounded-xl border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800 text-xs">
            </div>
          </div>
          ...
          <div class="grid grid-cols-2 gap-3">
            <div>
              ...
              <select v-model="prefForm.payout_frequency" class="w-full px-3 py-2 rounded-xl border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800 text-xs">
            ...
            <div>
              ...
              <input v-model="prefForm.payout_method" type="text" class="w-full px-3 py-2 rounded-xl border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800 text-xs" placeholder="Bank Transfer, PayPal, etc.">
            </div>
          </div>
          ...
          <div class="grid grid-cols-3 gap-2">
            <div>
              ...
              <input v-model="prefForm.bank_name" type="text" placeholder="Chase, Wells, etc." class="w-full px-3 py-2 rounded-xl border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800 text-xs">
            </div>
            <div>
              <label class="block text-xs font-semibold text-slate-700 dark:text-slate-300 mb-1">Account #</label>
              <input v-model="prefForm.account_number" type="text" class="w-full px-3 py-2 rounded-xl border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800 text-xs">
            </div>
            <div>
              <label class="block text-xs font-semibold text-slate-700 dark:text-slate-300 mb-1">Routing #</label>
              <input v-model="prefForm.routing_number" type="text" class="w-full px-3 py-2 rounded-xl border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800 text-xs">
            </div>
          </div>

          <div class="flex items-center gap-2 pt-1">
            <input v-model="prefForm.vip_partner" type="checkbox" id="vip_partner_check" class="rounded border-slate-300 text-emerald-600 focus:ring-emerald-500">
            <label for="vip_partner_check" class="text-xs font-semibold text-slate-700 dark:text-slate-300 cursor-pointer">
          ...
          <div>
            ...
            <textarea v-model="prefForm.notes" rows="3" placeholder="Agreed vehicle inspection schedules, bonuses..." class="w-full px-3 py-2 rounded-xl border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800 text-xs"></textarea>
          </div>

          <div class="flex justify-end gap-2 pt-2">
            <button type="button" @click="showPreferenceModal = false" class="px-4 py-2 rounded-xl border border-slate-200 dark:border-slate-700 text-xs">Cancel</button>
            <button type="submit" :disabled="prefForm.processing" class="px-4 py-2 rounded-xl bg-emerald-600 hover:bg-emerald-700 text-white font-semibold text-xs">Save Settings</button>
          </div>
        </form>
      </div>
    </div>
