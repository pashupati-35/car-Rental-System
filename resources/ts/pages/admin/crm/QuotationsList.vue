  ...
  <CrmLayout>
    ...
    <div class="space-y-6 pb-12">
      ...
      <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 class="text-2xl font-bold text-slate-900 dark:text-white">Rental Quotations & Proposals</h1>
          ...
          <table class="w-full text-left text-sm text-slate-600 dark:text-slate-300">
            <thead class="bg-slate-50 dark:bg-slate-800/60 text-xs font-semibold uppercase tracking-wider text-slate-500 dark:text-slate-400 border-b border-slate-200 dark:border-slate-800">
              <tr>
                ...
                <th class="px-5 py-3.5">Client / Lead</th>
                <th class="px-5 py-3.5">Vehicle</th>
                <th class="px-5 py-3.5">Rental Period</th>
            ...
            <tbody class="divide-y divide-slate-100 dark:divide-slate-800">
              ...
              >
                ...
                <td class="px-5 py-4 text-xs">
                  <div v-if="quote.customer" class="font-bold text-slate-800 dark:text-slate-200">
                    {{ quote.customer.name }}
                  </div>
                  <div v-else-if="quote.lead" class="font-bold text-slate-800 dark:text-slate-200">
                    {{ quote.lead.first_name }} {{ quote.lead.last_name }} (Lead)
                  </div>
                  <div v-else class="text-slate-400">Direct Proposal</div>
                </td>

                <!-- Vehicle -->
                <td class="px-5 py-4 text-xs font-medium">
                  <div v-if="quote.car" class="flex items-center gap-1.5">
                    <i class="ri-car-line text-emerald-500" />
                    <span>{{ quote.car.brand_name }} {{ quote.car.name }}</span>
                  </div>
                  <span v-else class="text-slate-400">Custom Fleet</span>
                </td>
      ...
      <div v-if="showViewModal && selectedQuoteForView" class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-xs">
        <div class="bg-white dark:bg-slate-900 rounded-2xl max-w-2xl w-full p-8 border border-slate-200 dark:border-slate-800 shadow-2xl space-y-6 max-h-[90vh] overflow-y-auto">
          ...
          <div class="flex items-center justify-between border-b border-slate-100 dark:border-slate-800 pb-4">
            <div>
              ...
              <h2 class="text-xl font-extrabold text-slate-900 dark:text-white">Official Rental Quotation</h2>
              <p class="text-xs font-mono text-slate-400">{{ selectedQuoteForView.quotation_number }}</p>
            </div>
            <div class="flex items-center gap-2">
              <button
                type="button"
            ...
            <table class="w-full text-left">
              <thead class="bg-slate-50 dark:bg-slate-800 font-semibold text-slate-600 dark:text-slate-300">
                <tr>
                  ...
                  <th class="p-3 text-right">Daily / Unit</th>
                  <th class="p-3 text-right">Qty/Days</th>
              ...
              <tbody class="divide-y divide-slate-100 dark:divide-slate-800">
                <tr>
                  ...
                  <td class="p-3 text-right">${{ Number(selectedQuoteForView.daily_rate).toFixed(2) }}</td>
                  <td class="p-3 text-right">{{ selectedQuoteForView.days_count }} days</td>
                  <td class="p-3 text-right font-bold">${{ Number(selectedQuoteForView.subtotal).toFixed(2) }}</td>
                </tr>
              </tbody>
            </table>
          </div>
          ...
          <div class="flex justify-end text-xs">
            <div class="w-64 space-y-2">
              ...
              <div v-if="Number(selectedQuoteForView.discount_amount) > 0" class="flex justify-between text-rose-500">
          ...
          <!-- Terms -->
          <div v-if="selectedQuoteForView.terms_conditions" class="p-3.5 rounded-xl bg-slate-50 dark:bg-slate-800 text-[11px] text-slate-500 whitespace-pre-wrap">
