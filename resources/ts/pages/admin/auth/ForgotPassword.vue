...
<template>
  <AuthLayout>
    <Head title="Admin Password Recovery" />

    <template #title>
      <div class="flex items-center justify-center gap-2">
        <span class="inline-block w-2.5 h-2.5 rounded-full bg-indigo-600 animate-pulse" />
        Admin Password Reset
      </div>
    </template>
    <template #subtitle>
      Enter your administrator email to receive password reset instructions
    </template>
    ...
    <div class="space-y-4">
      ...
      >
        <div>
          <label class="block text-xs font-semibold uppercase tracking-wider text-gray-700 dark:text-gray-300 mb-1.5">Master Admin Email</label>
          <input
            v-model="form.email"
            type="email"
            required
        ...
        >
          <span v-if="form.processing">Sending Admin Recovery Link...</span>
        ...
        <div class="text-center pt-2">
          <Link
            href="/admin/login"
            class="text-xs text-gray-500 hover:text-indigo-600 dark:hover:text-gray-300 font-semibold transition-colors"
          >
