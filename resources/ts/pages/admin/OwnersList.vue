...
const ownersList = ref<any[]>(props.owners || [])
...
const fetchOwners = async () => {
  ...
  try {
    ...
    if (res.data?.data) {
      ownersList.value = res.data.data
    }
  } catch (err) {
...
const submitNewOwner = async () => {
  ...
  try {
    ...
    if (res.data?.status === 'success' || res.status === 200 || res.status === 201) {
      message.value = 'Fleet Owner registered successfully! Password setup email has been dispatched.'
      if (res.data?.reset_url) {
        generatedResetUrl.value = res.data.reset_url
      }
      form.value = { full_name: '', email: '', contact_number: '', address: '', gender: 'male' }
      fetchOwners()
    }
  } catch (err: any) {
    errorMessage.value = err.response?.data?.message || 'Failed to create fleet owner.'
  } finally {
...
<template>
  <AppLayout>
    <Head title="Fleet Owners - Admin Portal" />

    <template #header>
      Fleet Owners Directory
    </template>
      ...
      <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div>
          ...
          <p class="text-xs text-gray-500 mt-0.5">
            Manage owner accounts, fleet assignments, and onboard new car owners.
          </p>
        </div>

        <button
          class="px-4 py-2.5 rounded-xl bg-blue-600 hover:bg-blue-700 text-white font-semibold text-xs shadow-md shadow-blue-500/20 transition-all flex items-center gap-2"
        ...
        <table v-else class="w-full text-left text-sm">
          ...
          <tbody class="divide-y divide-gray-100 dark:divide-gray-800">
            ...
            >
              ...
              <td class="py-4 px-6">
                <span class="px-2.5 py-1 rounded-full text-xs font-semibold bg-blue-50 text-blue-700 dark:bg-blue-900/30 dark:text-blue-300">
                  {{ owner.cars_count ?? owner.cars?.length ?? 0 }} Cars
                </span>
              </td>
              <td class="py-4 px-6">
                <span
                  :class="owner.is_mfa_enabled ? 'bg-emerald-50 text-emerald-700 dark:bg-emerald-900/30 dark:text-emerald-300' : 'bg-gray-100 text-gray-600'"
                  class="px-2 py-0.5 rounded text-[11px] font-semibold"
                >
                  {{ owner.is_mfa_enabled ? 'MFA Active' : 'Off' }}
                </span>
              </td>
              <td class="py-4 px-6 text-xs text-gray-500">
      ...
      >
        <div class="bg-white dark:bg-gray-900 rounded-2xl max-w-lg w-full p-6 border border-gray-100 dark:border-gray-800 shadow-2xl relative">
          ...
          <p class="text-xs text-gray-500 mb-6">
            When you register a new owner, a secure password setup link will automatically be emailed to them.
          </p>
          ...
          <div v-if="message" class="mb-4 p-3 rounded-xl bg-emerald-50 border border-emerald-200 text-emerald-800 text-xs">
            ...
            <div v-if="generatedResetUrl" class="mt-2 pt-2 border-t border-emerald-200">
              ...
              <a :href="generatedResetUrl" target="_blank" class="text-[11px] font-mono text-blue-600 hover:underline break-all">
                {{ generatedResetUrl }}
              </a>
            </div>
          </div>
          ...
          <form class="space-y-4" @submit.prevent="submitNewOwner">
            <div>
            ...
            <div>
              ...
              <input
                ...
                type="text"
                required
                placeholder="123 Fleet Way, New York"
                class="w-full px-3.5 py-2.5 rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"
              >
            </div>
