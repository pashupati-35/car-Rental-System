...
const toggleEmailAuth = async () => {
  ...
  mfaSuccess.value = ''
  try {
    const url = emailAuthEnabled.value ? '/owner/mfa/email/deactivate' : '/owner/mfa/email/activate'
...
<template>
  <AppLayout>
    <Head title="Owner Account Security & MFA" />
    ...
    <div class="space-y-6 max-w-5xl mx-auto">
        ...
        <div>
          <div class="flex items-center gap-2">
            ...
            <h1 class="text-xl sm:text-2xl font-black text-slate-900 dark:text-white tracking-tight">
              Owner Security & Multi-Factor Auth
            </h1>
          </div>
          <p class="text-xs text-slate-500 mt-1">
            Manage two-factor authentication, email login codes, and portal password security.
          </p>
        </div>
      ...
      <div class="p-6 sm:p-7 rounded-3xl bg-white dark:bg-slate-900 border border-slate-200/80 dark:border-slate-800 shadow-xs space-y-6">
        <div>
          ...
          <p class="text-xs text-slate-500 mt-0.5">
            Add an extra layer of security using email verification or authenticator apps
          </p>
        </div>
          ...
          <div class="py-4.5 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            <div>
              ...
              <p class="text-xs text-slate-500">
                Receive verification codes via your registered owner email ({{ user?.email }})
              </p>
            </div>
            ...
            <div class="flex items-center gap-3 shrink-0">
              ...
              <button
                type="button"
                :class="emailAuthEnabled ? 'bg-rose-50 hover:bg-rose-100 text-rose-700 dark:bg-rose-950/40 dark:hover:bg-rose-900/60 border-rose-200 dark:border-rose-800' : 'bg-emerald-600 hover:bg-emerald-700 text-white shadow-xs'"
      ...
      <div class="p-6 sm:p-7 rounded-3xl bg-white dark:bg-slate-900 border border-slate-200/80 dark:border-slate-800 shadow-xs space-y-5">
        <div class="flex items-center gap-2">
          <i class="ri-lock-password-line text-lg text-emerald-600" />
          <h2 class="text-base font-extrabold text-slate-900 dark:text-white">
            Change Password
          </h2>
        </div>
      ...
      >
        ...
        >
          ...
          <div class="p-3 bg-slate-50 dark:bg-slate-800/80 rounded-xl space-y-1">
            ...
            <code class="text-xs font-mono font-bold text-emerald-600 dark:text-emerald-400 break-all select-all">{{ secretKey }}</code>
          </div>
          ...
          <div>
            ...
            <input
              ...
              maxlength="6"
              placeholder="000000"
              class="w-full text-center tracking-[0.3em] font-mono text-xl py-2.5 rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 focus:ring-2 focus:ring-emerald-500 focus:outline-none"
          ...
          <div class="flex gap-2 pt-2">
            ...
            <button
              type="button"
              class="flex-1 py-2.5 rounded-xl bg-emerald-600 hover:bg-emerald-700 text-white font-bold text-xs shadow-xs cursor-pointer"
              :disabled="mfaLoading"
