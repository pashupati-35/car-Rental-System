...
const submit = () => {
  form.post('/owner/reset-password', {
...
<template>
  <AuthLayout>
    <Head title="Owner Password Setup" />
    ...
    <template #subtitle>
      Enter your new password to access your fleet owner portal
    </template>
    ...
    <div class="space-y-4">
      ...
      >
        ...
        <button
          ...
          class="w-full py-3 px-4 rounded-xl bg-gradient-to-r from-emerald-600 to-teal-600 hover:from-emerald-700 hover:to-teal-700 text-white font-bold text-sm shadow-lg shadow-emerald-500/25 disabled:opacity-50 transition-all flex items-center justify-center gap-2 cursor-pointer"
        >
          <i
          ...
          <span v-else>Save Password & Sign In</span>
        </button>

        <div class="text-center pt-2">
          <Link
            href="/owner/login"
            class="text-xs text-gray-500 hover:text-emerald-600 dark:hover:text-gray-300 font-semibold transition-colors"
          >
            &larr; Return to Owner Login
          </Link>
        </div>
      </form>
    </div>
