...
const adminNav = [
  ...
  { title: 'Admin Profile & MFA', icon: 'ri-shield-user-line', href: '/admin/profile' },
]
...
const customerNav = [
  ...
  { title: 'Profile & Security', icon: 'ri-user-line', href: '/customer/profile' },
]
  ...
  <VApp :theme="isDark ? 'dark' : 'light'">
    ...
    >
      <div class="p-4 flex items-center gap-3 border-b border-gray-100 dark:border-gray-800">
        <div class="w-10 h-10 rounded-xl bg-gradient-to-br from-blue-600 to-indigo-600 flex items-center justify-center text-white font-bold text-xl shadow-md shadow-blue-500/30">
          CR
        </div>
        <div>
          <h2 class="font-bold text-gray-900 dark:text-white leading-tight text-sm">
            Car Rental
          </h2>
          <span
            :class="role === 'Admin' ? 'bg-indigo-50 text-indigo-700' : role === 'Owner' ? 'bg-emerald-50 text-emerald-700' : 'bg-blue-50 text-blue-700'"
      ...
      <template #append>
        <div class="p-4 border-t border-gray-100 dark:border-gray-800 space-y-2">
          <button
            class="w-full flex items-center justify-center gap-2 py-2.5 px-4 rounded-xl text-red-600 bg-red-50 hover:bg-red-100 dark:bg-red-950/40 dark:hover:bg-red-900/60 font-medium text-xs transition-all"
            @click="logout"
          >
            Sign Out
          </button>
    ...
    >
      ...
      <VAppBarTitle class="text-sm font-semibold text-gray-800 dark:text-white">
        <slot name="header">
      ...
      <div class="flex items-center gap-3">
        <div class="flex items-center gap-2 ps-2 border-l border-gray-200 dark:border-gray-700">
          ...
          <span class="text-xs font-semibold text-gray-700 dark:text-gray-200 hidden sm:inline">{{ displayName }}</span>
        </div>
      </div>
    </VAppBar>
    ...
    <VMain class="bg-gray-50/50 dark:bg-gray-950">
      ...
      >
        <div class="p-4 rounded-xl bg-emerald-50 border border-emerald-200 text-emerald-800 flex items-center gap-3 shadow-sm text-xs font-medium">
      ...
      <div
        ...
        class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mt-4"
      >
        <div class="p-4 rounded-xl bg-rose-50 border border-rose-200 text-rose-800 flex items-center gap-3 shadow-sm text-xs font-medium">
