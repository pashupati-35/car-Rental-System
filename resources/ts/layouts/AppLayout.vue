...
<template>
  <v-app :theme="isDark ? 'dark' : 'light'">
    <v-navigation-drawer v-model="drawer" elevation="2" class="border-r border-gray-200 dark:border-gray-800">
      <div class="p-4 flex items-center gap-3 border-b border-gray-100 dark:border-gray-800">
        ...
        <div>
          <h2 class="font-bold text-gray-900 dark:text-white leading-tight">Car Rental</h2>
      ...
      <v-list density="comfortable" class="p-2 space-y-1">
        <v-list-item
    ...
    <v-app-bar flat border="b" class="bg-white/80 dark:bg-gray-900/80 backdrop-blur-md px-4">
      <v-app-bar-nav-icon @click="drawer = !drawer" />
      
      <v-app-bar-title class="text-base font-semibold text-gray-800 dark:text-white">
        <slot name="header">Car Rental Dashboard</slot>
      </v-app-bar-title>
      ...
      <div class="flex items-center gap-3">
        ...
        >
          <i :class="isDark ? 'ri-sun-line text-yellow-500' : 'ri-moon-line'"></i>
        </button>
    ...
    <v-main class="bg-gray-50/50 dark:bg-gray-950">
