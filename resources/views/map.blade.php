...
<head>
    ...
    <style>
        #map { height: 400px; width: 50%; margin: 0 auto; }
...
<!-- <input type="text" id="distance_traveled" readonly>km -->
...
<script>
    $(document).ready(function() {
        ...
        var map = L.map('map').setView([27.7172, 85.3240], 12); // Kathmandu, Nepal
        ...
        function fetchSuggestions(input, suggestionsContainer, latInput, lngInput, markerType) {
        ...
        function updateMapCenter() {
            // If both pickup and drop locations are set, fit the map to show both
            if (pickupMarker && dropMarker) {
                var bounds = L.latLngBounds(pickupMarker.getLatLng(), dropMarker.getLatLng());
                map.fitBounds(bounds, { padding: [50, 50] });
            } else if (pickupMarker) {
                map.setView(pickupMarker.getLatLng(), 14);
            } else if (dropMarker) {
                map.setView(dropMarker.getLatLng(), 14);
            }
        }
            ...
            if (!isNaN(lat1) && !isNaN(lng1) && !isNaN(lat2) && !isNaN(lng2)) {
                ...
                fetch(url)
                    ...
                    .then(data => {
                        ...
                        var routeBounds = routeLayer.getBounds();
